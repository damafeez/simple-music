...
class Player extends StatelessWidget {
  ...
  Player(
      {Key key, this.panPercent, this.onPanUpdate, this.navigationLogicEvents})
      : super(key: key);

  _onPlaylistTap({scrollToTop = false,}) {
    ...
    scrollController.animateTo(
      scrollController.offset == scrollBottom || scrollToTop ? scrollTop : scrollBottom,
  ...
  Widget build(BuildContext context) {
    return Consumer<MusicEngine>(
      builder: (context, musicEngine, child) {
        return Scaffold(
          body: musicEngine.currentSongIndex < 0
              ? Container()
              : ListView(
